...
# ===========================================================================
# DynamicDPSGDSchedule — property tests
# ===========================================================================

# Fixed privacy params shared across all DynamicDPSGD property tests so JAX
# does not recompile for every Hypothesis example (T=10 matches _T_FIXED).
_DYN_PARAMS = GDPPrivacyParameters(1.0, 0.126936737507, 0.1, _T_FIXED)

# Strategies for the three learnable scalars.  Values are kept in [0.1, 3.0]
# to avoid numerical edge-cases in __find_mu_0 (Brent bracket [tol, 100]).
...
# Default clamp value used throughout the property tests.
_DYN_EPS = 0.01
...
class TestDynamicDPSGDScheduleProperties:
    ...
    def test_clips_always_positive(self, rho_mu, rho_C, c_0):
        ...
        s = _make_dynamic(rho_mu, rho_C, c_0)
        assert jnp.all(s.get_private_clips() > 0)
    ...
    def test_clips_correct_shape(self, rho_mu, rho_C, c_0):
        ...
        s = _make_dynamic(rho_mu, rho_C, c_0)
        assert s.get_private_clips().shape == (_T_FIXED,)
    ...
    def test_clips_finite(self, rho_mu, rho_C, c_0):
        ...
        s = _make_dynamic(rho_mu, rho_C, c_0)
        assert jnp.all(jnp.isfinite(s.get_private_clips()))
    ...
    def test_project_clamps_rho_mu_to_eps(self, rho_mu, rho_C, c_0):
        ...
        s = _make_dynamic(rho_mu, rho_C, c_0)
        assert float(s.project().rho_mu) >= _DYN_EPS - 1e-8
    ...
    def test_project_clamps_rho_C_to_eps(self, rho_mu, rho_C, c_0):
        ...
        s = _make_dynamic(rho_mu, rho_C, c_0)
        assert float(s.project().rho_C) >= _DYN_EPS - 1e-8
    ...
    def test_project_clamps_C_0_to_eps(self, rho_mu, rho_C, c_0):
        ...
        s = _make_dynamic(rho_mu, rho_C, c_0)
        assert float(s.project().C_0) >= _DYN_EPS - 1e-8
    ...
    def test_project_clips_positive(self, rho_mu, rho_C, c_0):
        ...
        s = _make_dynamic(rho_mu, rho_C, c_0)
        assert jnp.all(s.project().get_private_clips() > 0)

    # --- mu_0 consistency after project ---
